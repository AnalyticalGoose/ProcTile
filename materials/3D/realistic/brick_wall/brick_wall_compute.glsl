...
layout(rgba32f, set = 1, binding = 0) uniform restrict image2D rgba32f_buffer;
...
layout(r16f, set = 1, binding = 4) uniform restrict image2D noise_buffer;
layout(r16f, set = 1, binding = 5) uniform restrict image2D grunge_buffer;
...
layout(set = 5, binding = 0, std430) buffer restrict readonly MortarColour {
	vec4 mortar_col;
};
...
void main() {
    ...
    if (params.stage == 1.0) {
        ...
        vec3 random_brick_colour = mix(vec3(0.0, 0.0, 0.0), hash_ws3(vec2(float((seed.brick_colour_seed)), hash12(vec2(hash12(brick_fill.xy), hash12(brick_fill.zw))))), step(0.0000001, dot(brick_fill.zw, vec2(1.0))));
